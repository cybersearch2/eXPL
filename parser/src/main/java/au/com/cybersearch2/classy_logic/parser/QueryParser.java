...
public class QueryParser implements QueryParserConstants 
{
  ...
  protected Operand axiomContainerOperand(
          ...
          Operand expression)
  {
      return axiomContainerOperand(parserAssembler, null, indexData, expression);
  }
      ...
      Operand expression)
  {
    ...
    ListItemVariable operand = name == null ? new ListItemVariable(qname, indexData, expression) : new ListItemVariable(name, qname, indexData, expression);
  ...
  protected Operand listItemOperand(
      ...
      ListItemSpec indexData,
      Operand expression) throws ParseException
  {
      return listItemOperand(context, null, indexData, expression);
  }
  ...
  Template createDynamicAxiomTemplate(String listName, ParserContext context)
  {
    ...
    return context.getParserAssembler().getTemplateAssembler().createTemplate(qualifiedTemplateName, TemplateType.calculator);
  }
  ...
  {
    VariableType varType = new VariableType(OperandType.AXIOM, parserAssembler);
    ItemList<?> itemList = varType.getItemListInstance(listName.getName());
    if (isPublic)
      itemList.setPublic(true);
    return listName.getName();
  }

  void  setBuiltinList(QualifiedName qname, ListItemSpec[] listIndexData, ParserContext context)
  {
    ParserAssembler parserAssembler = context.getParserAssembler();
    ListAssembler listAssembler = parserAssembler.getListAssembler();
    QualifiedName listName = listAssembler.getAxiomListMapping(qname);
    listIndexData[0].setQualifiedListName(listName);
    if (listIndexData.length > 1)
      listIndexData[1].setQualifiedListName(listName);
  }
...
/** Root production. */
  ...
  Token typeToken = null;
    ...
    if (typeToken == null)
      {if (true) return;}
    ParserAssembler parserAssembler = context.getParserAssembler();
    VariableType varType = typeToken.kind == QueryParserConstants.AXIOM ? new VariableType(OperandType.AXIOM, parserAssembler) : new VariableType(OperandType.TERM, parserAssembler);
    varType.setProperty(VariableType.AXIOM_KEY, querySpec.getKey());
    ItemList<?> itemList = varType.getItemListInstance(querySpec.getName());
    itemList.setPublic(true);
  ...
  QualifiedName contextName = context.getContextName();
    ...
    if (typeToken == null)
      {if (true) return;}
    ParserAssembler parserAssembler = context.getParserAssembler();
    VariableType varType = new VariableType(typeToken.kind == QueryParserConstants.AXIOM ? OperandType.AXIOM : OperandType.TERM, parserAssembler);
    varType.setProperty(VariableType.AXIOM_KEY, template.getQualifiedName());
    ItemList<?> itemList = varType.getItemListInstance(template.getName());
    itemList.setPublic(true);
    parserAssembler.getListAssembler().addItemList(itemList.getQualifiedName(), itemList);
  }
  ...
  Token templateToken;
    ...
    {if (true) return context.getParserAssembler().getTemplateAssembler().createTemplate(qualifiedTemplateName, isCalculator ? TemplateType.calculator : TemplateType.template);}
    throw new Error("Missing return statement in function");
  }
  ...
  ParserAssembler parserAssembler = context.getParserAssembler();
    ...
    case IDENTIFIER:
      ...
      break;
    case CHOICE:
      selectToken = jj_consume_token(CHOICE);
      ...
      case LBRACE:
        ...
        case LPAREN:
          ...
          case UNKNOWN:
          case IDENTIFIER:
            parameterTemplate = Parameters(nameToken.image, context, false);
            break;
          default:
            jj_la1[47] = jj_gen;
            ;
        ...
        context.setSourceMarker(selectToken, innerTemplate.getQualifiedName());
          ...
          case CURRENCY:
          case IDENTIFIER:
            var = TemplateTerm(innerTemplate, context);
            break;
          case INTEGER_LITERAL:
          case FLOATING_POINT_LITERAL:
          case STRING_LITERAL:
        ...
        {if (true) return selectionOperand;}
          break;
        default:
          jj_la1[54] = jj_gen;
          jj_consume_token(-1);
      ...
      {if (true) return createChoiceOperand(QualifiedName.parseName(nameToken.image), template, parameterTemplate, context);}
      break;
    case REGEX:
      regexOp = RegularExpression(context);
      {if (true) return new Evaluator(regexOp.getLeftOperand().getQualifiedName(), regexOp, "?", regexOp.getLeftOperand());}
      break;
    case SCOPE:
      scopeToken = jj_consume_token(SCOPE);
    ...
    var = listItemOperand(context, listIndexData[0], null);
    context.onTokenIntercept(scopeToken);
    {if (true) return var;}
      break;
    default:
      jj_la1[56] = jj_gen;
  ...
  final public Operand TemplateTerm(Template template, ParserContext context) throws ParseException
  {
  ...
  Template parameterTemplate = null;
    ...
    isList = (varType == null) &&
             (listAssembler.existsKey(ListType.basic, qname) ||
              ...
              listAssembler.existsKey(ListType.axiom_dynamic, qname));
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) 
    {
    case LBRACKET:
    case RARROW:
      ...
      else if (listAssembler.existsKey(ListType.builtin, qname))
        setBuiltinList(qname, listIndexData, context);
      break;
    default:
      jj_la1[58] = jj_gen;
      ;
      ...
      case PLUSASSIGN:
        ...
        case AXIOM:
          ...
          axiomList = AxiomList(axiomQname, context);
            VariableType axiomListVarType = new VariableType(OperandType.LIST, parserAssembler);
            axiomListVarType.setProperty(VariableType.PARAMS, axiomList);
            if (axiomQname != null)
              axiomListVarType.setProperty(VariableType.AXIOM_KEY, axiomQname);
            expression = axiomListVarType.getInstance(new QualifiedName(qname.getName() + qname.incrementReferenceCount(), qname));
          break;
        case SCOPE:
        case INTEGER_LITERAL:
        case FLOATING_POINT_LITERAL:
    ...
    if (parameterTemplate != null)
      {if (true) return parserAssembler.getCallOperand(qname, parameterTemplate);}

    if (isList || (axiomList != null))
      {if (true) return new VariableType(OperandType.APPENDER, parserAssembler).createAppender(qname, operator, expression);}
    ...
    if (!isDeclaration)
      varType = new VariableType(OperandType.UNKNOWN, parserAssembler);
    Operand assignExpression = (assignToken != null) ? expression : null;
    if (assignExpression != null)
      varType.setProperty(VariableType.REFLEX_OP, OperatorEnum.convertOperator(assignToken.image));
    if (expression != null)
      varType.setProperty(VariableType.EXPRESSION, expression);
    if (listIndexData !=null)
      var = listIndexData.length == 1 ? listItemOperand(context, listIndexData[0], expression) : axiomContainerOperand(parserAssembler, listIndexData, expression);
    ...
    if ((var != null) && (assignToken != null))
      var = parserAssembler.createReflexiveEvaluator(var, assignToken.image, expression);
    if ((var == null) && !hasOperand)
      var = varType.getInstance(name);
    if (var == null)
    ...
    if (hasOperand && (equalsToken != null))
  ...
  OperandMap operandMap = parserAssembler.getOperandMap();
    ...
    if (jj_2_3(2)) 
    {
      group = GroupDeclaration(qname.getName(), parserAssembler);
    } else 
    {
      ;
    }
    ...
    if (inputOp == null)
      inputOp = operandMap.addOperand(qname.getName(), null, parserAssembler.getQualifiedContextname());
  ...
  StringBuilder builder = new StringBuilder();
    ...
    template = parserAssembler.getTemplateAssembler().createTemplate(qualifiedChoiceName, TemplateType.choice);
    ...
    while (true) 
    {
      ...
      jj_consume_token(PLUS);
      listName = ChoiceListDeclaration(context);
        context.setQualifiedName(nameToken.image, listName);
    }
    jj_consume_token(LPAREN);
  ...
  final public String ChoiceListDeclaration(ParserContext context) throws ParseException
  {
  Token nameToken;
  Token scopeToken;
  String listName;
  ...
  ListAssembler listAssembler = parserAssembler.getListAssembler();
    ...
    listAssembler.mapAxiomList(parserAssembler.getContextName(listName), new QualifiedName(scopeToken.image, QualifiedName.EMPTY, listName));
  ...
  final public Group GroupDeclaration(String name, ParserAssembler parserAssembler) throws ParseException
  {
  Group group = new Group(name);
    jj_consume_token(LBRACE);
    Group(group, parserAssembler);
    label_24:
    while (true) 
    {
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) 
  ...
  final public void Group(Group group, ParserAssembler parserAssembler) throws ParseException
  {
  Token groupToken;
  ...
  context.setSourceItemPending(true);
    ...
    default:
     ...
     if (isUntyped)
       varType = new VariableType(OperandType.UNKNOWN, parserAssembler);
     if (expression != null) //  Need literal condition? && !(expression instanceof Evaluator, parserAssembler)
       varType.setProperty(expression.isEmpty() ? VariableType.EXPRESSION : VariableType.LITERAL, expression);
     Operand operand = varType.getInstance(name);
     {if (true) return operand;}
    throw new Error("Missing return statement in function");
  }
  ...
  QualifiedName qualifiedTargetName = null;
  ...
  ListAssembler listAssembler = parserAssembler.getListAssembler();
    ...
    listName = nameToken.image;
    if (scopeToken != null)
      qualifiedAxiomName = new QualifiedName(scopeToken.image, QualifiedName.EMPTY, listName);
    ...
    case ASSIGN:
      ...
      varType = new VariableType(axiomList.isEmpty() ? OperandType.AXIOM : OperandType.LIST, parserAssembler);
      if (axiomList.isEmpty())
        {if (true) return createAxiomList(qualifiedAxiomName, exportToken !=null, parserAssembler);}
      ...
      operand = varType.getInstance(qualifiedAxiomName);
    ...
    if (sourceItem != null)
      sourceItem.setEnd(delimitToken);
    if (scopeToken != null)
      listAssembler.mapAxiomList(parserAssembler.getContextName(listName), qualifiedAxiomName);
    if ((axiomToken != null) || (target != null) || (scopeToken != null))
      {if (true) return listName;}
    ...
    if (varType == null)
      varType = new VariableType(OperandType.AXIOM, parserAssembler);
    if (qualifiedAxiomName != null)
      varType.setProperty(VariableType.AXIOM_KEY, qualifiedAxiomName);
    ItemList<?> itemList;
    if (template != null)
      itemList = varType.getDynamicListInstance(listName, template);
    else
      itemList = varType.getItemListInstance(listName);
    if (exportToken !=null)
      itemList.setPublic(true);
    if (beginToken != null)
  ...
  final public void CursorDeclaration(ParserContext context) throws ParseException
  {
  Token keywordToken;
  Token nameToken;
  Token listToken;
  ...
  ListAssembler listAssembler = parserAssembler.getListAssembler();
    ...
    jj_consume_token(RPAREN);
    delimitToken = jj_consume_token(SEMICOLON);
    QualifiedName listName = null;
    if (scopeToken != null)
      listName = new QualifiedName(scopeToken.image, QualifiedName.EMPTY, listToken.image);
    else
      listName = context.getQualifiedName(listToken.image);
    if (scopeToken != null)
      listAssembler.mapAxiomList(parserAssembler.getContextName(cursorName), listName);
  ...
  if (varType == null)
    ...
    if (varType.getOperandType() == expression.getOperator().getTrait().getOperandType())
      var = expression;
    if (var == null)
      var = varType.getInstance(expression.getQualifiedName(), expression);
  ...
  Operand assignOperand = null;
    ...
    if ((var == null) && assignToken.image.equals("="))
      {if (true) return new Evaluator(param, assignToken.image, assignOperand);}
    if (var == null)
      {if (true) return context.getParserAssembler().createReflexiveEvaluator(param, assignToken.image, assignOperand);}
    if (assignToken.image.equals("+="))
      var.setDoConcatenate(true);
    var.setExpression(assignOperand);
    {if (true) return var;}
    throw new Error("Missing return statement in function");
  }
  ...
  OperandMap operandMap = parserAssembler.getOperandMap();
    ...
    case IDENTIFIER:
      ...
      if (jj_2_4(2)) 
      {
        param1 = NamedExpression(qname, context);
      } else 
      {
        ;
      }
    if (param1 != null)
      {if (true) return param1;}
      ListAssembler listAssembler = parserAssembler.getListAssembler();
      QualifiedName inContext = new QualifiedName(qname.toString(), context.getContextName());
      if (listAssembler.existsKey(ListType.basic, inContext) ||
          listAssembler.existsKey(ListType.axiom_item, inContext) ||
          listAssembler.existsKey(ListType.axiom_dynamic, qname))
        {if (true) return new VariableType(OperandType.APPENDER, parserAssembler).createAppender(inContext, null, null);}
      {if (true) return parserAssembler.addOperand(qname);}
      break;
    case SCOPE:
    ...
    if (listIndexData.length > 1)
         {if (true) throw new ParseException("Scope cannot be accessed using axiom list variable");}
    operand = listItemOperand(context, listIndexData[0], null);
  ...
  ParserAssembler parserAssembler = context.getParserAssembler();
    ...
    case INTEGER:
      ...
      varType = new VariableType(OperandType.INTEGER, parserAssembler);
      context.onTokenIntercept(literalToken);
      {if (true) return varType;}
      break;
    case BOOLEAN:
      literalToken = jj_consume_token(BOOLEAN);
      varType = new VariableType(OperandType.BOOLEAN, parserAssembler);
      context.onTokenIntercept(literalToken);
      {if (true) return varType;}
      break;
    case DOUBLE:
      literalToken = jj_consume_token(DOUBLE);
      varType = new VariableType(OperandType.DOUBLE, parserAssembler);
      context.onTokenIntercept(literalToken);
      {if (true) return varType;}
      break;
    case STRING:
      literalToken = jj_consume_token(STRING);
      varType = new VariableType(OperandType.STRING, parserAssembler);
      context.onTokenIntercept(literalToken);
      {if (true) return varType;}
      break;
    case DECIMAL:
      literalToken = jj_consume_token(DECIMAL);
      varType = new VariableType(OperandType.DECIMAL, parserAssembler);
      context.onTokenIntercept(literalToken);
      {if (true) return varType;}
      break;
    case TERM:
      literalToken = jj_consume_token(TERM);
      varType = new VariableType(OperandType.TERM, parserAssembler);
      context.onTokenIntercept(literalToken);
      {if (true) return varType;}
      break;
    case CURRENCY:
      ...
      varType = new VariableType(OperandType.CURRENCY, parserAssembler);
  ...
  final public Operand NamedExpression(QualifiedName qualifiedName, ParserContext context) throws ParseException
  {
  ListItemSpec[] indexData;
  ParserTask parserTask;
  Template parametersTemplate = null;
  QualifiedName listName = context.getQualifiedName(qualifiedName.toString());
  ParserAssembler parserAssembler = context.getParserAssembler();
  QualifiedName qname = parserAssembler.getContextName(qualifiedName.toString());
  ListAssembler listAssembler = parserAssembler.getListAssembler();
    switch ((jj_ntk==-1)?jj_ntk():jj_ntk) 
    {
    case LBRACKET:
    case RARROW:
      indexData = IndexExpression(listName, context);
    if (parserAssembler.getListAssembler().existsKey(ListType.context, qname))
      indexData[0].getQualifiedListName().toContextName();
    else if (listAssembler.existsKey(ListType.builtin, listName))
      setBuiltinList(listName, indexData, context);
    {if (true) return indexData.length == 1 ?
    listItemOperand(context, indexData[0], null) :
    axiomContainerOperand(parserAssembler, indexData, null);}
      break;
    case LPAREN:
      jj_consume_token(LPAREN);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) 
      ...
      case UNKNOWN:
      case IDENTIFIER:
        parametersTemplate = Parameters(qualifiedName.toString(), context, true);
        break;
      default:
        jj_la1[119] = jj_gen;
        ;
    ...
    case DOT:
      ...
      Operand operand = parserAssembler.addOperand(qualifiedName);
      qname = parserAssembler.getContextName(qualifiedName.toString() + "_format");
      ...
      Operand target = parserAssembler.addOperand(qualifiedName);
      {if (true) return new FactOperand(target);}
        break;
      default:
        jj_la1[120] = jj_gen;
        jj_consume_token(-1);
  ...
  OperandMap operandMap = parserAssembler.getOperandMap();
    ...
    case IDENTIFIER:
      ...
      identifier = jj_consume_token(IDENTIFIER);
        listName = context.getQualifiedName(identifier.image);
      switch ((jj_ntk==-1)?jj_ntk():jj_ntk) 
      {
      case LBRACKET:
      case RARROW:
        listIndexData = IndexExpression(listName, context);
          ListAssembler listAssembler = parserAssembler.getListAssembler();
          if (listAssembler.existsKey(ListType.context, parserAssembler.getContextName(listName.getName())))
            listIndexData[0].getQualifiedListName().toContextName();
          else if (listAssembler.existsKey(ListType.builtin, listName))
            setBuiltinList(listName, listIndexData, context);
        break;
      default:
        jj_la1[124] = jj_gen;
        ;
    ...
    default:
     ...
     if (listIndexData !=null)
       {if (true) return listIndexData.length == 1 ? listItemOperand(context, name, listIndexData[0], null) : axiomContainerOperand(parserAssembler, name, listIndexData, null);}
     ...
     if (expression != null)
       varType.setProperty(VariableType.EXPRESSION, expression);
     {if (true) return varType.getInstance(name);}
    throw new Error("Missing return statement in function");
  }
  ...
  private boolean jj_3R_64() {
    if (jj_scan_token(canon_eq)) return true;
    return false;
  }

  private boolean jj_3R_63() {
    if (jj_scan_token(unicode_case)) return true;
    return false;
  }

  private boolean jj_3R_62() {
    if (jj_scan_token(dotall)) return true;
    return false;
  }
  ...
  private boolean jj_3R_60() {
    if (jj_scan_token(multiline)) return true;
    return false;
  }
  ...
  private boolean jj_3R_58() {
    if (jj_scan_token(case_insensitive)) return true;
    return false;
  }

  private boolean jj_3R_57() {
    if (jj_scan_token(unix_lines)) return true;
    return false;
  }
  ...
  {
    jj_la1_0 = new int[] { 0x800000,0x1547dfc0,0x1547dfc0,0x0,0x0,0x10001000,0x0,0x0,0x0,0x15475fc0,0x15475fc0,0x401000,0x0,0x0,0x0,0x0,0x0,0x15465fc0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x14007c0,0x14007c0,0x0,0x14007c0,0x401000,0x0,0x0,0x10140000,0x0,0x0,0x0,0x401000,0x0,0x0,0x10140000,0x540a7c0,0x0,0x540a7c0,0x0,0x0,0x0,0x14007c0,0x14007c0,0x0,0x14007c0,0x0,0x0,0x0,0x0,0x0,0x540a7c0,0x14007c0,0x0,0x0,0x0,0x9000,0x0,0x0,0x14007c0,0x0,0x0,0x0,0x2000,0x0,0x0,0x540a7c0,0x540a7c0,0x540a7c0,0x0,0x0,0xe0000000,0xe0000000,0x14007c0,0x0,0x0,0x540a7c0,0x0,0x540a7c0,0x0,0x0,0x0,0x0,0x8000,0x0,0x0,0x0,0x0,0x0,0x14007c0,0x0,0x0,0x10000000,0x14017c0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x8000,0x0,0x0,0x0,0x0,0x0,0x14007c0,0x0,0x14007c0,0x2280000,0x0,0x0,0x14007c0,0x0,0x14007c0,0x0,0x0,0x14007c0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x8000,0x8000,0x0,0x0, };
  }
  private static void jj_la1_init_1() 
  {
    jj_la1_1 = new int[] { 0x0,0x10000,0x10000,0x1000000,0x20000,0x0,0x20000000,0x0,0x20000,0x10000,0x10000,0x0,0x40000000,0x4000000,0x1000000,0x20000,0x0,0x10000,0x80000,0x80000,0x80000,0x1000000,0x10000,0x0,0x1000000,0x0,0x17a40,0x1fa40,0x80000,0x17a40,0x0,0x40000000,0x0,0x0,0x2000000,0x1000000,0x2000000,0x0,0x40000000,0x0,0x0,0xa0b7a40,0x1000000,0xa0b7a40,0x20000,0x0,0x0,0x17a40,0x17a40,0x0,0x17a40,0x0,0x0,0x1000000,0xa0000,0xa0000,0x10000,0x0,0x4200000,0x20000000,0x10000,0x37a40,0x0,0x0,0x17a40,0x200a0000,0x200a0000,0x0,0x0,0x80000,0x2000000,0x20b7a40,0x37a40,0xb7a40,0x20000000,0x10800,0x3f,0x3f,0x17a40,0x4000000,0x1000000,0xa0b7a40,0x1000000,0xa0b7a40,0x0,0x1000000,0x80000,0x1000000,0x37a40,0x1000000,0x1000000,0x10000,0x1000000,0x1000000,0x0,0x4200000,0x20000000,0x0,0x0,0x40000000,0x10000000,0x10040,0x90000,0x20000,0x200a0000,0x200a0000,0x40000000,0x10000000,0x1000000,0x20000000,0x20000000,0x37a40,0x4200000,0x2200000,0x7a40,0x10800,0x0,0x0,0x12000000,0x17a40,0x0,0x6220000,0x1000000,0x0,0x4200000,0x17a40,0x20000000,0x7a40,0x10000,0x20000,0x3000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0xc0000000,0xc0000000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x37a40,0x37a40,0x0,0x0, };
  }
  private static void jj_la1_init_2() 
  {
    jj_la1_2 = new int[] { 0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x2,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x2,0x0,0x0,0x2,0x2,0x0,0x0,0x10000,0x10000,0x10000,0x0,0x0,0x0,0x0,0x0,0x0,0x800,0x0,0x0,0x0,0x0,0x0,0x0,0x800,0x0,0x1e07,0x0,0x1e07,0x0,0x6,0x6,0x0,0x0,0x6,0x0,0x6,0x6,0x0,0x0,0x0,0x0,0x0,0x0,0x400000,0x0,0x1e01,0xff800000,0x1,0x0,0xffc00001,0xffc00001,0x6,0x6,0x0,0x0,0x1e07,0x1e01,0x1e01,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1e07,0x0,0x1e07,0x800,0x0,0x0,0x0,0x1e01,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0xffc00000,0xffc00000,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x80,0x100,0x10000,0x20000,0x8000,0x48,0x48,0x30,0x30,0x380000,0x380000,0x1800,0x1800,0x46000,0x46000,0x1800,0x1e01,0x1,0x600,0x600, };
  }
  private static void jj_la1_init_3() 
  {
    jj_la1_3 = new int[] { 0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4,0x0,0x4,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4,0x1,0x0,0x0,0x1,0x1,0x0,0x0,0x0,0x0,0x4,0x4,0x4,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4,0x0,0x4,0x0,0x0,0x0,0x0,0x4,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x1,0x1,0x0,0x0,0x0,0x0,0x0,0x2,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x4,0x4,0x0,0x0,0x0, };
  }
